...
/*   Updated: 2024/06/06 23:14:47 by mregrag          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

void	delete_env_var(t_env **head, char *var)
{
	t_env	*temp;
	t_env	*prev;

	temp = *head;
	if (temp && !ft_strncmp(temp->env, var, ft_strlen(var)))
	{
		*head = temp->next;
		free(temp);
		temp = NULL;
	...
	while (temp && ft_strncmp(temp->env, var, ft_strlen(var)))
	{
		prev = temp;
		temp = temp->next;
	}
